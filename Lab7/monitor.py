...
def main(minutes_to_pull, source_service, destination_service, training_data_file, scraper_port):

    start_http_server(scraper_port)
    ...
    url_test_datab = f'histogram_quantile(0.5, sum by (le) (rate(istio_request_duration_milliseconds_bucket{{app="{source_service}", destination_app="{destination_service}", reporter="source"}}[1m])))'
    test_urlb = "http://34.135.43.19:9090"
    #test_urlb=f"http://34.135.43.19:{prometheus_port}"
    ...
    g_req50 = Gauge(f"lab7_{source_service}_2_{destination_service}_req_50", "request seconds")

    g_req50.set(0)
    
    #lab7_frontend_2_shippingservice_anomaly_count

    #lab7_checkoutservice_2_shippingservice_y_max

    #mae_metric = Gauge("prophet_mae", "Mean Absolute Error for Prophet Model")
    mae_metric = Gauge(f"lab7_{source_service}_2_{destination_service}_MAE", "Mean Absolute Error for Prophet Model")
    mape_metric = Gauge(f"lab7_{source_service}_2_{destination_service}_MAPE", "Mean Absolute Percentage Error for Prophet Model")
    anomaly_metric = Gauge(f"lab7_{source_service}_2_{destination_service}_anomaly_count", "Number of Anomalies Detected")
    y_min_metric = Gauge(f"lab7_{source_service}_2_{destination_service}_y_min", "Min Value of y in Test Data")
    y_metric = Gauge(f"lab7_{source_service}_2_{destination_service}_y", "Actual Value of y in Test Data")
    y_max_metric = Gauge(f"lab7_{source_service}_2_{destination_service}_y_max", "Max Value of y in Test Data")
...
if __name__ == '__main__':
    if len(sys.argv) != 6:
        print("Usage: python3 monitor.py <minutes_to_pull> <source_service> <destination_service> <training_data_file> <scraper_port>")
        sys.exit(1)
    
    minutes_to_pull = int(sys.argv[1])
    source_service = sys.argv[2]
    destination_service = sys.argv[3]
    training_data_file = sys.argv[4]
    scraper_port = int(sys.argv[5])

    main(minutes_to_pull, source_service, destination_service, training_data_file, scraper_port)
